  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-8 border-l-4 border-primary pl-6">
        <div>
          <h2 className="text-3xl font-black text-brand-dark uppercase tracking-tight">Kreator Publikacji</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Step {step} of 2: {step === 1 ? 'Powiązanie z katalogiem' : 'Finalna weryfikacja'}</p>
        </div>
        ...
        <div className="space-y-8">
          <Card className="bg-slate-50 border-none shadow-inner overflow-hidden">
            <CardContent className="p-8">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Twój Wygenerowany Przedmiot</h3>
              <p className="text-xl font-black text-brand-dark italic tracking-tight">{dispensary.title}</p>
            </CardContent>
          </Card>
      ...
      {step === 2 && (
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="border-none shadow-2xl bg-slate-50">
              <CardContent className="p-8 space-y-4">
                <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Dane Lokalnego Systemu</h4>
                <p className="text-lg font-black text-brand-dark uppercase tracking-tight">{dispensary.title}</p>
            ...
            <Card className="border-none shadow-2xl bg-primary/5 border-2 border-primary/20">
              <CardContent className="p-8 space-y-4">
                <h4 className="text-[10px] font-black uppercase text-primary tracking-widest">Powiązany Produkt Platform</h4>
                <p className="text-lg font-black text-brand-dark uppercase tracking-tight">{selectedProduct?.name}</p>
                <div className="flex items-center gap-4 p-4 bg-white rounded-lg border border-primary/10">
                  ...
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{selectedProduct?.category?.name || 'Inne'}</p>
          ...
          <div className="flex items-center justify-between pt-10">
            <button 
              onClick={() => setStep(1)} 
              className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-brand-dark transition-colors"
            >

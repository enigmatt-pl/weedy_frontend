...
import { Search, MapPin, Clock, Star, Package, Loader2, SlidersHorizontal, X, Leaf } from 'lucide-react';
            ...
            {filtered.map((dispensary) => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-3xl border border-slate-100 shadow-lg shadow-slate-200/50 overflow-hidden group cursor-pointer hover:shadow-2xl hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all duration-500"
                >
                  {/* Image */}
                  <div className="aspect-[4/3] bg-gradient-to-br from-emerald-50 to-slate-100 overflow-hidden relative">
                    {images.length > 0 ? (
                      <img
                        src={images[0]}
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                        ...
                        <span className="text-xs font-medium text-slate-300">Brak zdjęcia</span>
                      </div>
                    )}
                    ...
                    {images.length > 1 && (
                      <div className="absolute bottom-3 right-3 bg-black/40 backdrop-blur-sm text-white text-[9px] font-bold px-2 py-1 rounded-full">
                        +{images.length - 1} zdjęć
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="text-base font-bold text-brand-dark leading-tight group-hover:text-primary transition-colors line-clamp-1">
                        {dispensary.title}
                      </h3>
                      <div className="flex items-center gap-1 shrink-0">
                        <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                        <span className="text-xs font-bold text-slate-700">4.{(dispensary.id % 4) + 5}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                      <div className="flex items-center gap-1.5 text-slate-400">
                        ...
                        <span className="text-xs font-medium">Pon-Sob 9–21</span>
                      </div>
